...
// Same treatment as the other high-traffic reads: the expenses page is a
// sidebar navigation target, and every visit previously re-ran its ~4 query
// render against the remote DB. Categories change rarely (300s); the list /
// rollups (15s) are purged immediately whenever an expense action runs its
...
export const listExpenseCategories = unstable_cache(
  async (gymId: string): Promise<ExpenseCategoryItem[]> => {
...
export const listExpenses = unstable_cache(
  async (params: {
    gymId: string;
    search?: string;
    categoryId?: string;
...
export const getMonthlyCategoryBreakdown = unstable_cache(
  async (
    gymId: string,
    monthStart: Date,
    monthEnd: Date,
...
export const getPnlSummary = unstable_cache(
  async (gymId: string, monthStart: Date): Promise<PnlSummary> => {
